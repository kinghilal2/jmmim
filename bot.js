...
if (message.author.id !== '405215218453446668') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
client.user.setGame(argresult);
...
client.on('message', message => {
if(message.content.includes("<@405215218453446668>")) {
...
client.on("message", msg=>{
let id = "405215218453446668"; // ايديك
...
`).then( msgs =>{
const filter = response => response.author.id == "405215218453446668" && response.mentions._content.includes(`:moneybag: | ${msg.author.username}, has transferred \`$${Price2}\` to ${msg.guild.members.get(id)}`);
